...
insert into Scout (lastName, firstName, teamId, isActive, emailAddress, isAdmin) select 'K', 'Kirill', t.id, 'Y', 'kraskaska.dev.2Bscouting@gmail.com', 'Y' from Team t where t.teamNumber = 2526 and not exists (select 1 from Scout s where s.lastName = 'K' and s.firstName = 'Kirill' and s.teamId = t.id);
...
-- 3926 MPArors
insert into Scout (lastName, firstName, teamId, isActive, emailAddress, isAdmin) select '(Choose Scout)', '2926', t.id, 'Y', '3926@x.com', 'N' from Team t where t.teamNumber = 2526 and not exists (select 1 from Scout s where s.lastName = '(Choose Scout)' and s.firstName = '3926' and s.teamId = t.id);
