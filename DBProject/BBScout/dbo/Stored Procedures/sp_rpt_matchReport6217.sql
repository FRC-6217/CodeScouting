...
CREATE PROCEDURE sp_rpt_matchReport6217 (@pv_matchId int
...
BEGIN
    ...
    select m.type + ' ' + m.number matchNumber
		...
		, case when @pv_is6217 = 1
			...
			else '<a href="../Reports/robotReport6217.php?TeamId=' + convert(varchar, tm.teamId) + '"> ' + convert(varchar, t.teamNumber) + '</a> '
			end teamReportUrl
		, asr.matchCnt
	 ...
	 from Match m
		...
		left outer join TeamGameEvent tge
