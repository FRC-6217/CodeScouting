...
$tsql = "select m.id matchId
		...
		group by m.id
			...
			order by 7 desc
					, abs(convert(integer,
						...
						case when tm.alliance = 'R' then m.redScore - m.redFoulPoints - m.redAlliancePoints
								else m.blueScore - m.blueFoulPoints - m.blueAlliancePoints end) desc
					, m.datetime, tm.alliance;";
    $getResults = sqlsrv_query($conn, $tsql);
    if ($getResults == FALSE)
		if( ($errors = sqlsrv_errors() ) != null) {
			foreach( $errors as $error ) {
    ...
    while ($row = sqlsrv_fetch_array($getResults, SQLSRV_FETCH_ASSOC)) {
		echo "<tr>";
			...
			if ($autoAuditFunction == "Y") {
				if ($row['nbrSRs'] == 3) {
					echo "<td><a href='/Reports/matchAuditReport6217.php?matchId=" . $row['matchId'] . "&alliance=" . substr($row['alliance'], 0, 1) . "&matchNumber=" . $row['matchNumber'] . "'>Submit</a></td>";
				}
				else {
					echo "<td></td>";
				}
